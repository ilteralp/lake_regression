...
"""
Returns min, max and means of confusion matrices. 
"""
def _get_vmin_vmax_means(conf_mats):
    ...
    for cm in conf_mats:
        ...
        means.append(np.mean(np.diag(cm)))
    return vmin, vmax, means
...
def visualize_conf_mats(conf_mats, names, best_fold):
    ...
    vmin, vmax, means = _get_vmin_vmax_means(conf_mats)
    ...
    for i, ax in enumerate(axn.flat):
        ...
        ax.set_xlabel('Mean: {:.2f}'.format(means[i]))
