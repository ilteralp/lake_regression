...
def train_random_on_folds(model, dataset, unlabeled_dataset, train_fn, args, report, fold_sample_ids):
    ...
    if args['num_folds'] is not None:
        ...
        for fold in range(args['num_folds']):
            ...
            # for n in ['tr_ids', 'val_ids', 'test_ids']:
            #     print('{}, len: {}'.format(n, len(fold_sample_ids[n][fold])))
            # print('val, {}'.format(fold_sample_ids['val_ids'][fold]))
            # print('test, {}'.format(fold_sample_ids['test_ids'][fold]))
            # print('+' * 72)
...
if __name__ == "__main__":
    ...
    # fold_sample_ids = {'tr_ids': [np.array([268, 257, 147, 285, 255, 130, 307, 182, 265,  26, 110,  23, 238,
    #      296, 279,  92, 217, 275,  95, 131, 124, 215, 282,  46, 318,  91,
    #       65, 101, 150, 259, 160,  71, 235,  57, 174,  74, 177,  39, 264,
    #      132, 143,  56, 104, 228, 108, 190,   7, 153,  15, 269, 175, 261,
    #      133, 158, 180, 144, 313,  83, 157, 186,  78,  40, 234,  47, 114,
    #      254, 140, 206,  96,  94,  16,  89,  11,  66, 198, 237, 116, 253,
    #        4,  97, 266, 185, 245,   3, 304, 284, 295, 187,  69, 193, 194,
    #       80, 202, 305, 173, 163, 242, 127, 277, 136, 286,  13, 273, 159,
    #      117, 311, 220,  98, 263, 113, 209, 139, 224, 154, 250,   0, 303,
    #       33, 166, 236,  22, 230,  36,  60,   8,  18, 212, 168,  67,  87,
    #      172, 169, 298, 246,  24, 256, 315, 258, 219,  79, 213,  25,   1,
    #       28, 243,  42, 247,  45, 111, 252,  37, 294,  76, 308, 240, 167,
    #       55, 171,  19, 281, 290, 276, 103, 207, 107, 125,  84,  58,  35,
    #      288, 119, 289,  62, 109, 317,  53, 155, 128, 226,  10, 126, 262,
    #      205,  86, 165,  52,  27, 302, 301, 260, 297, 225,  81, 314, 218,
    #      199, 280, 138, 121, 244, 148,  61, 122, 309, 293, 216, 310,  73,
    #       72, 229, 248, 232,  34, 270,  49,   2,   5, 200, 191, 283,  12,
    #      105, 115,  75, 274,  88, 123, 201,  43,  70, 267,  63, 118, 292,
    #      151,  48, 179, 271, 178, 272, 189, 161, 222, 112,  17, 196,  59,
    #      149,  51, 100,  90,  38,  41,  32,  85, 291])],
    #     'test_ids': [np.array([ 64,  93, 164, 197, 251, 142, 241, 287,  30,   9, 204, 183,  54,
    #      227, 233, 170,  68, 152, 106, 306, 210,  14,  50, 156,  44, 134,
    #      192, 319, 300, 249, 120,  31])],
    #     'val_ids': [np.array([214, 223, 312, 181, 239,  82, 316, 211,  99, 299, 203, 129, 221,
    #      208, 162, 184, 141,  21,   6,  29,  20, 102, 135, 188, 195, 137,
    #      231,  77, 176, 146, 278, 145])]}
    ...
    for (loss_name, fold_setup, pred_type, unlabeled, date_type, split_layer, patch_size, patch_norm) in itertools.product(loss_names, fold_setups, pred_types, using_unlabeled_samples, date_types, split_layers, patch_sizes, patch_norms):
        ...
        # if SAMPLE_IDS_FROM_RUN_NAME is None and args['fold_setup'] != prev_setup_name:                                 # New fold_setup, old sample ids are meaningless now.
        if args['fold_setup'] != prev_setup_name:
            print('fold_sample_ids are None due to moving from {} to {}.'.format(prev_setup_name, args['fold_setup'])) # Only works if SAMPLE_IDS_FROM_RUN_NAME is not given. 
